...
extern "C" {
    ...
    */
    typedef struct
    {
		...
		unsigned long tiltCompOutsideLimit : 1;  //with active tilt compensation tilt too large
		...
		unsigned long reserved1 : 12;            // reserviert
	...
	typedef struct
	{
		...
		AdcWeight value;				// this structure is only valid if type == ADC_TARE_KNOWN
		AdcPercent percentage;			// this structure is only valid if type == ADC_TARE_PERCENT
	} AdcTare;
